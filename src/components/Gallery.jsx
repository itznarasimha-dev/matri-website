  ...
  return (
    <section className="py-14 sm:py-20 lg:py-28" style={{ background: '#ffffff' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 auto-rows-[140px] sm:auto-rows-[180px] lg:auto-rows-[200px]">
          {photos.map((p, i) => (
            <div key={i} className={`relative overflow-hidden rounded-xl group cursor-pointer ${p.span}`}>
              ...
              <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center"
                ...
                <span className="font-body text-white font-semibold text-xs sm:text-sm tracking-wide uppercase">{p.label}</span>
              </div>
            </div>
          ))}
        </div>
