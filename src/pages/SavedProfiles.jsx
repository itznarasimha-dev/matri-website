    ...
    <DashLayout>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={{ background: '#fff', borderRadius: 16, padding: '18px 22px', boxShadow: '0 1px 12px rgba(35,56,176,0.07)', border: '1px solid #E8EBF5', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <div>
            <h1 style={{ fontFamily: 'Cormorant Garamond, serif', fontWeight: 700, fontSize: 26, color: '#1A1F36', margin: 0 }}>Saved Profiles</h1>
            <p style={{ fontFamily: 'Outfit, sans-serif', fontSize: 13, color: '#9CA3AF', margin: 0 }}>{saved.length} profile{saved.length !== 1 ? 's' : ''} shortlisted</p>
          </div>
          <Link to="/matches" style={{ padding: '9px 20px', borderRadius: 10, background: '#2338B0', color: 'white', fontFamily: 'Outfit, sans-serif', fontSize: 13, fontWeight: 600, textDecoration: 'none', boxShadow: '0 4px 14px rgba(35,56,176,0.25)' }}>
            Browse More
          </Link>
        </div>
        ...
        {saved.length > 0 && (
          <div style={{ position: 'relative' }}>
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search saved profiles..."
              style={{ width: '100%', padding: '10px 14px 10px 40px', borderRadius: 12, border: '1px solid #E8EBF5', fontFamily: 'Outfit, sans-serif', fontSize: 13, color: '#374151', background: '#fff', outline: 'none', boxSizing: 'border-box' }} />
          </div>
        )}

        {/* Empty state */}
        {saved.length === 0 ? (
          <div style={{ background: '#fff', borderRadius: 16, padding: '60px 20px', textAlign: 'center', boxShadow: '0 1px 12px rgba(35,56,176,0.07)', border: '1px solid #E8EBF5' }}>
            ...
            <p style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 600, fontSize: 16, color: '#374151', marginBottom: 8 }}>No saved profiles yet</p>
            <p style={{ fontFamily: 'Outfit, sans-serif', fontSize: 13, color: '#9CA3AF', marginBottom: 20 }}>Browse matches and shortlist profiles you like.</p>
            <Link to="/matches" style={{ padding: '10px 24px', borderRadius: 10, background: '#2338B0', color: 'white', fontFamily: 'Outfit, sans-serif', fontSize: 13, fontWeight: 600, textDecoration: 'none' }}>Browse Matches</Link>
          </div>
        ) : filtered.length === 0 ? (
          <div style={{ background: '#fff', borderRadius: 16, padding: '40px 20px', textAlign: 'center', border: '1px solid #E8EBF5' }}>
            <p style={{ fontFamily: 'Outfit, sans-serif', color: '#9CA3AF' }}>No profiles match "{search}"</p>
          </div>
        ) : (
