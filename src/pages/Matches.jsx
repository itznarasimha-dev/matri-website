...
const Label = ({ children }) => (
  <div style={{ fontFamily: 'inherit', fontSize: 11, fontWeight: 600, color: '#6B7280', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 6 }}>{children}</div>
)

const Select = ({ value, onChange, options }) => (
  <select value={value} onChange={e => onChange(e.target.value)}
    style={{ width: '100%', padding: '8px 12px', borderRadius: 10, border: '1px solid #E8EBF5', fontFamily: 'inherit', fontSize: 13, color: '#374151', background: 'white', outline: 'none' }}>
    ...
    <DashLayout>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={{ background: '#fff', borderRadius: 16, padding: '18px 22px', boxShadow: '0 1px 12px rgba(0,0,128,0.07)', border: '1px solid #E8EBF5', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <div>
            <h1 style={{ fontFamily: 'inherit', fontWeight: 700, fontSize: 26, color: '#1A1F36', margin: 0 }}>Browse Matches</h1>
            <p style={{ fontFamily: 'inherit', fontSize: 13, color: '#9CA3AF', margin: 0 }}>{filtered.length} profiles found for you</p>
          </div>
          <button onClick={() => setFiltersOpen(o => !o)}
            style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 18px', borderRadius: 10, fontFamily: 'inherit', fontSize: 13, fontWeight: 600, border: '1px solid #E8EBF5', cursor: 'pointer', transition: 'all 0.15s',
              background: filtersOpen ? '#000080' : 'white', color: filtersOpen ? 'white' : '#374151' }}>
        ...
        <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start' }}>
          ...
          {/* Filter panel */}
          {filtersOpen && (
            <div style={{ width: 220, flexShrink: 0, background: '#fff', borderRadius: 16, padding: 18, boxShadow: '0 1px 12px rgba(0,0,128,0.07)', border: '1px solid #E8EBF5', position: 'sticky', top: 24 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                <span style={{ fontFamily: 'inherit', fontWeight: 700, fontSize: 16, color: '#1A1F36' }}>Filters</span>
                <button onClick={() => setFilters({ gender: 'any', ageMin: 18, ageMax: 40, caste: 'Any', location: 'Any', education: 'Any' })}
                  style={{ fontFamily: 'inherit', fontSize: 11, color: '#000080', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}>Reset</button>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    {['any', 'bride', 'groom'].map(g => (
                      <button key={g} onClick={() => set('gender', g)}
                        style={{ flex: 1, padding: '6px 0', borderRadius: 8, fontFamily: 'inherit', fontSize: 11, fontWeight: 600, textTransform: 'capitalize', border: '1px solid #E8EBF5', cursor: 'pointer', transition: 'all 0.15s',
                          background: filters.gender === g ? '#000080' : 'white',
                ...
                <div>
                  ...
                  <input type="range" min={18} max={50} value={filters.ageMin} onChange={e => set('ageMin', +e.target.value)} style={{ width: '100%', accentColor: '#000080', marginBottom: 6 }} />
                  <input type="range" min={18} max={50} value={filters.ageMax} onChange={e => set('ageMax', +e.target.value)} style={{ width: '100%', accentColor: '#000080' }} />
                </div>
          ...
          <div style={{ flex: 1, display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 16 }}>
            {filtered.length === 0 ? (
              <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: '60px 20px', background: '#fff', borderRadius: 16, border: '1px solid #E8EBF5' }}>
                ...
                <p style={{ fontFamily: 'inherit', color: '#9CA3AF' }}>No profiles match your filters.</p>
              </div>
            ) : filtered.map(p => (
              <div key={p.id}
