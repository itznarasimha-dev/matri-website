...
function HomeLanding() {
  return (
    <main>
      ...
      <section className="py-16 md:py-24" style={{ background: '#2C1654' }}>
...
function WelcomeBar({ user }) {
  ...
  return (
    <div className="relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #2C1654 0%, #3D1F6B 100%)' }}>
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            <img src={user.photo} alt={user.name} className="w-14 h-14 rounded-full object-cover"
              style={{ border: '2px solid #D4A017', boxShadow: '0 0 20px rgba(212,160,23,0.3)' }} />
            <div>
              <p className="font-body text-sm mb-0.5" style={{ color: '#D4A017' }}>{greeting} 🪷</p>
              ...
              <p className="font-body text-sm mt-1" style={{ color: 'rgba(255,255,255,0.55)' }}>
                You have{' '}
                <span style={{ color: '#D4A017', fontWeight: 600 }}>{user.newInterests} new interests</span> and{' '}
                <span style={{ color: '#D4A017', fontWeight: 600 }}>{user.matchesCount} matches</span> waiting
              </p>
            </div>
          </div>
  ...
  return (
    <div ref={ref} className="reveal rounded-3xl p-7" style={{ background: '#F5ECD7', border: '1px solid #E8D5B0' }}>
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="font-display font-bold text-2xl" style={{ color: '#7B2FBE' }}>{pct}%</span>
      </div>
      <div className="h-2.5 rounded-full mb-2 overflow-hidden" style={{ background: '#E8D5B0' }}>
        <div className="h-full rounded-full" style={{ width: `${pct}%`, background: 'linear-gradient(90deg, #7B2FBE, #D4A017)', transition: 'width 1s cubic-bezier(0.16,1,0.3,1)' }} />
      </div>
      ...
      <div className="space-y-2 mb-6">
        {nudges.map(n => (
          <div key={n.label} className="flex items-center justify-between">
            ...
            <span className="font-body text-xs font-bold" style={{ color: '#7B2FBE' }}>{n.points}</span>
          </div>
        ))}
      </div>
...
function MembershipNudge() {
  ...
  return (
    <div ref={ref} className="reveal rounded-3xl p-7 relative overflow-hidden"
      style={{ background: 'linear-gradient(135deg, #7B2FBE 0%, #3D1F6B 100%)', border: '1px solid rgba(212,160,23,0.3)' }}>
      ...
      <div className="flex items-start gap-3 mb-5">
        <Star className="w-5 h-5 shrink-0 mt-0.5" style={{ color: '#D4A017' }} />
        <div>
...
function HomeDashboard({ user }) {
  ...
  return (
    <main style={{ background: '#2C1654', minHeight: '100vh' }}>
            ...
            {user.newInterests > 0 && (
              <section>
                <div className="flex items-end justify-between mb-6">
                  ...
                  <Link to="/interests" className="font-body text-sm flex items-center gap-1" style={{ color: '#D4A017' }}>
            ...
            {interestedIn.length > 0 && (
              <section>
                <div className="flex items-end justify-between mb-6">
                  ...
                  <Link to="/saved" className="font-body text-sm flex items-center gap-1" style={{ color: '#D4A017' }}>
                    View All <ChevronRight className="w-4 h-4" />
                  </Link>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
